...
class TemporalBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, dilation, dropout):
        ...
        # If in_channels != out_channels, downsample for the residual connection.
    ...
    def forward(self, x):
        out = self.conv1(x)
        # Trim the extra padding to maintain causality.
...
class TCN(nn.Module):
    def __init__(self, num_inputs, num_channels, kernel_size=3, dropout=0.2, num_layers=3, dilation_base=2, num_output_neurons=4, seed_value=16981):
        ...
        layers = []
        in_channels = 1  # Treat the input as a single-channel sequence.
        for i in range(num_layers):
            out_channels = num_channels  # Use the same number of filters for each layer.
            dilation = dilation_base ** i
            layers.append(TemporalBlock(in_channels, out_channels, kernel_size, dilation, dropout))
            in_channels = out_channels
        self.network = nn.Sequential(*layers)
        # Final fully connected layer: flatten output and map to desired output.
    ...
    def forward(self, x):
        ...
        x = x.unsqueeze(1)
        y = self.network(x)
        # Flatten the output: (batch, num_channels * seq_len)
        y = y.view(y.size(0), -1)
        return self.fc(y)
