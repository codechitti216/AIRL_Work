...
# Import the LSTM model from LSTM.py
...
def plot_velocity_predictions(predictions, traj, beam_fill_window, title_suffix=""):
    ...
    Debug prints show first five values for predicted, ground truth, and moving average per beam.
    """
...
def main():
    ...
    for idx, traj_pair in enumerate(training_trajectory_pairs):
        ...
        # Removed: cumulative_trained_on.append(f"{traj}:{traj_epochs}")
    ...
    final_checkpoint_filename = f"LSTM_{run_id}_{missing_freq_str}_final.pth"
