...
class MemoryNeuralNetwork(nn.Module):
    ...
    def forward(self, input_array):
        # Detach the input to ensure a fresh computational graph.
        ...
        self.input_to_last_layer_nn = (
            torch.matmul(self.output_of_hidden_layer_nn, self.weights_hidden_to_output_nn) +
            torch.matmul(self.output_of_hidden_layer_nn, self.weights_hidden_to_output_mn) +
            (self.beta * self.prev_output_of_nn)
        )
        
        self.output_nn = self.input_to_last_layer_nn
        
        # Detach the persistent state to avoid retaining gradients from previous forward passes.
        self.prev_output_of_nn = self.output_nn.detach()
