...
from TCN import TemporalConvolutionalNetwork

CONFIG_PATH = "TCN.json"
RESULTS_PATH = "../../Experiments/Results_New/TCN/Results_TCN.csv"
CHECKPOINT_DIR = "../../Experiments/Results_New/TCN/Checkpoints_TCN/"
PLOTS_DIR = "../../Experiments/Results_New/TCN/Plots/"
LOG_FILE = "experiment_log_TCN.txt"
DATA_DIR = "../../../Data"
...
def train_model(trajectory_id, data_file, a, b):
    ...
    num_inputs = (6 * a) + (4 * b)
    input_data = torch.from_numpy(data.iloc[:-1, :num_inputs].values).float().pin_memory()
    target_data = torch.from_numpy(data.iloc[1:, -3:].values).float().pin_memory()
    ...
    while attempt < number_of_trials:
        model = TemporalConvolutionalNetwork(
            ...
            learning_rate_2=learning_rate_2,
        ...
        checkpoint_path = os.path.join(
            CHECKPOINT_DIR, 
            f"TCN_lr{learning_rate}_drop{dropout_rate}_{trajectory_id}_hid{hidden_neurons}_ep{epochs}_stack{stacking_count}_a{a}_b{b}.pth"
        )
        ...
        for epoch in range(epochs):
            ...
            for i in tqdm(range(len(input_data)), desc=f"Epoch {epoch + 1}/{epochs}"):
                x = input_data[i].to("cuda", non_blocking=True)
                y = target_data[i].to("cuda", non_blocking=True)
                
                optimizer.zero_grad()
                
                
                x = x.view(1, num_inputs, -1)  
        ...
        plot_path = os.path.join(PLOTS_DIR, f"TCN_lr{learning_rate}_drop{dropout_rate}_{trajectory_id}_hid{hidden_neurons}_ep{epochs}_stack{stacking_count}_a{a}_b{b}.png")
        plt.figure()
        ...
        if best_rmse <= rmse_threshold:
            ...
            log_message(f"[WARNING] RMSE {best_rmse:.5f} exceeds threshold. Adjusting hyperparameters...")
            learning_rate *= 1.1
            dropout_rate *= 1.05
